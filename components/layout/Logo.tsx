import React from 'react'

interface LogoProps {
  variant?: 'light' | 'dark'
  height?: number
  showText?: boolean
  className?: string
}

export default function Logo({ variant = 'light', height = 36, showText = true, className }: LogoProps) {
  const fillColor = variant === 'dark' ? '#fff' : undefined

  return (
    <div className={`flex items-start gap-3${className ? ' ' + className : ''}`}>
      <svg
        viewBox="0 0 1453.33 425.33"
        height={32}
        style={{ width: 'auto', flexShrink: 0, display: 'block', marginTop: '4px' }}
        aria-label="AutoAnkauf Baden Logo"
      >
        <path
          d="M312.12,114.42c-64.75,114.24-94.6,181.59-166.98,310.48-.8-1.29-146.52,2.49-145.13-1.33C1.39,422.74,227.01-.03,229.43.54c0,0,165.45,0,165.45,0,.27.55,115.49,200.33,114.43,201.11-13.83,2.3-31.75,8.31-36.89,9.19-32.91,8.32-62.31,17.84-90.7,25.48,0,0-69.61-121.9-69.61-121.9Z"
          fill={fillColor || '#0369A1'}
        />
        <path
          d="M542.23,195.29c-3.69-.61,106.88-193.84,107.45-194.67,1.03.78,177.83-1.15,178.36.49,25.53,45.92,58.86,98.43,84.07,147.81-44.02,2.88-69.27,4.09-109.63,7.7-12.76.69-26.14,4.14-37.48,2.43,0,0-22.21-44.61-22.21-44.61-.27-.57-1.14-.58-1.42,0,0,0-27.18,52.41-27.18,52.41-.4.77-1.19,1.25-2.05,1.26-66.59,7.88-90.2,11.22-169.91,27.19Z"
          fill={fillColor || '#FB6F6F'}
        />
        <path
          d="M1069.98,424.92c-.74-.45-165.65.86-165.99-.45-.9-.28-87.59-167.58-89.23-166.56-7.51,4.36-17.82,9.88-26.61,15.63-50.89,31.53-87.3,64.78-124.58,115.2-8.12,11.23-16.14,23.07-24.06,35.51-.66,1.93-137.9.01-139.12.67-.45,0-.72-.59-.4-.92,9.39-9.95,20.43-23.96,31.26-34.93,7.76-7.84,16.32-17.56,24.52-25.31,12.19-10.96,14.43-14.63,23.6-21.64.39-.23,1.2-1.34.38-1.54,0,0-71.57-3.14-71.57-3.14-3.72,1.14-6.16,3.99-11.54,7.93-35.14,27.13-57.2,50.49-86.11,79.49-86.12-.42-155.3,1.1-217.89-.53,25.03-19.73,54.83-41.24,82.04-57.15,136.8-80.17,265.41-124.18,416.22-156.89,204.61-41.44,361.81-46.89,575.5-45.29,17.38.44,38.81-12.7,38.29-33.03.58-9.72-2.69-21.75-10.56-28.32-39.25-18.9-65.86-4.54-234.71-8.79-1.6.25-4.4-.58-5.25.73-.52.55,1,51-.54,50.9-34.54-.53-65.38,1.32-102.05,1.55-37.6-64.05-36.78-64.72-83.93-145.96,122.83-4.48,250.76-.06,376.16-1.58,21.81-.47,50.77.34,68.85,3.61,42.49,6.63,87.36,31.45,100.58,74.94,14.06,43.19,6.22,102.34-38.41,124.89-.99.33-.57,1.46.27,1.51,12.11,3.6,24.48,8.41,34.7,15.61,74.57,51.06,48.56,181.62-41.78,197.02-20.55,4.86-41.49,7.27-64.37,6.9-42.05-.08-107.08-.08-194.87-.04-1.69-1.35-2.07-4.54-3.82-6.72-44.67-77.09-83.49-148.24-118.71-207.6-.71.1-35.98,4.85-36,4.86-2.06.94,121.31,208.52,119.73,209.44ZM695.51,236.95c.92-.18,44.43,2.13,45.23,1.52,26.27-12.23,52.99-23.65,80.26-31.92,14.3-6.53-38.71,3.21-42.72,4.21-33.68,7.19-37.09,9.5-72.4,20.6-1.96,1.04-14.33,3.56-10.37,5.59ZM706.23,253.35c-20.52,2.85-52.38-1.69-64.8,2.86-41.7,19.74-71.46,36.37-107.74,62.17,22.16,1.25,52.46,1.51,75.41.94,36.25-27.39,47.46-35.02,80.18-54.44,11.92-7.31,20.52-9.8,16.95-11.53ZM1100.76,257.76v68.7c0,.77.62,1.39,1.39,1.39h185.02c20.48,0,37.09-14.91,37.09-33.31,2.15-20.5-14.38-38.18-37.09-38.17-1.39,1.35-186.7-2.61-186.41,1.39Z"
          fill={fillColor || '#0EA5E9'}
        />
      </svg>

      {showText && (
        <div
          className="flex flex-col justify-center"
          style={{ gap: '2px' }}
        >
          <div className="flex items-baseline gap-0.5">
            <span className="font-semibold" style={{ color: '#0369A1', fontSize: '14.7px' }}>auto</span>
            <span className="font-semibold" style={{ color: '#FB6F6F', fontSize: '14.7px' }}>ankauf</span>
            <span className="font-semibold" style={{ color: '#0EA5E9', fontSize: '14.7px', marginLeft: '2px' }}>baden</span>
          </div>
          <span style={{ color: '#94A3B8', fontSize: '12.6px', lineHeight: 1 }}>
            Schnell. Einfach. In deiner Region.
          </span>
        </div>
      )}
    </div>
  )
}